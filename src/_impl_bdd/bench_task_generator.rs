...
pub fn spawn_tasks_2(left: &Bdd, right: &Bdd) -> (usize, usize) {
    let mut stack = Vec::with_capacity(max(left.size(), right.size()));
    stack.push((left.root_pointer(), right.root_pointer()));
    ...
    let mut op_cache: DynamicOpCache = DynamicOpCache::new(capacity);
    ...
    while let Some((l, r)) = stack.pop() {
        if op_cache.contains(l, r) {
            ...
            let c = op_cache.contains2((l_low, r_low), (l_high, r_high));
            if !c.0 {
                stack.push((l_low, r_low));
            }
            if !c.1 {
...
fn n_log_n(left: u64, right: u64) -> u64 {
    debug_assert!(left > 0);
